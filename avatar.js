...
function decorate(){clearTimeout(decorateTimer);decorateTimer=setTimeout(()=>{try{
  const mine=me();
  const myAv=document.querySelector('#page-mypage .my-avatar');if(myAv&&mine.id){myAv.innerHTML=window.YA(mine,'🌿');myAv.style.overflow='hidden'}
  document.querySelectorAll('#nanum-social-root .ys-av').forEach(el=>{const id=idFromOnclick(el,'socialPerson');if(id)el.innerHTML=window.YA(userById(id),'🌿')});
  document.querySelectorAll('#nanum-social-root .ys-person').forEach(el=>{const id=idFromOnclick(el,'socialPerson');if(!id)return;const u=userById(id);el.innerHTML=window.YAchip(u,u.emoji||'🌿')+' '+esc(u.name||'')});
  document.querySelectorAll('#nanum-social-root .ys-story').forEach(el=>{let id=idFromOnclick(el,'openStoryUser');if(!id&&/openStoryUploader/.test(el.getAttribute('onclick')||''))id=mine.id||'';const face=el.querySelector('.ys-face');if(id&&face)face.innerHTML=window.YA(userById(id),'🌿')});
  const top=document.querySelector('#ysStoryView .ys-story-top');if(top){const nm=top.querySelector('.nm'),face=top.querySelector('.face'),name=(nm&&nm.textContent||'').split(' · ')[0].trim(),u=userByName(name);if(face&&u)face.innerHTML=window.YA(u,'🌿')}
