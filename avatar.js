...
function userById(id){if(!id)return null;if(me().id===id)return me();return users().find(u=>String(u.id)===String(id))||{id:id,name:''};}
function userByName(name){if(!name)return null;if(me().name===name)return me();return users().find(u=>u.name===name)||null;}
function setHtml(el,html){if(el&&el.innerHTML!==html)el.innerHTML=html;}
function defaultAvatar(){return '<span class="ya-default" aria-label="기본 프로필"><svg viewBox="0 0 64 64" aria-hidden="true"><circle class="ya-default-head" cx="32" cy="24" r="12"></circle><path class="ya-default-body" d="M12 58c1.8-13.2 9.6-20 20-20s18.2 6.8 20 20H12z"></path></svg></span>'}
...
.ya-default{width:100%;height:100%;border-radius:50%;background:#f1f1f1;display:flex;align-items:center;justify-content:center;overflow:hidden}.ya-default svg{width:78%;height:78%;display:block}.ya-default-head,.ya-default-body{fill:#b8b8b8}
.ya-chip{display:inline-block;width:15px;height:15px;border-radius:50%;overflow:hidden;line-height:15px;text-align:center;vertical-align:-3px}
.ya-preview{width:96px;height:96px;border-radius:50%;margin:6px auto 14px;background:#f1f1f1;display:flex;align-items:center;justify-content:center;overflow:hidden}
#page-community .community-avatar,#page-community .community-comment-av{overflow:hidden;background:#f1f1f1!important}
...
window.YA=function(user){const id=user&&typeof user==='object'?(user.id||user.authorId||''):String(user||''),p=id&&map[id];if(p)return '<img class="ya-img" src="'+esc(publicUrl(p))+'" alt="" loading="lazy">';return defaultAvatar()};
window.YAchip=function(user){return '<span class="ya-chip">'+window.YA(user)+'</span>'};
...
function decorateCommunity(){
  document.querySelectorAll('#page-community .community-post').forEach(post=>{
    const name=(post.querySelector('.community-name')?.textContent||'').trim(),u=userByName(name)||{};
    const av=post.querySelector('.community-avatar');if(av)setHtml(av,window.YA(u));
    post.querySelectorAll('.community-comment').forEach(row=>{
      const cn=(row.querySelector('.community-comment-name')?.textContent||'').trim(),cu=userByName(cn)||{};
      const cav=row.querySelector('.community-comment-av');if(cav)setHtml(cav,window.YA(cu));
    });
  })
}
function decorate(){clearTimeout(decorateTimer);decorateTimer=setTimeout(()=>{try{
  ...
  document.querySelectorAll('#nanum-social-root .ys-person').forEach(el=>{const id=idFromOnclick(el,'socialPerson');if(!id)return;const u=userById(id);setHtml(el,window.YAchip(u)+' '+esc(u.name||''))});
...
function repaint(){try{if(typeof refreshSocialFeed==='function')refreshSocialFeed()}catch(e){}try{if(typeof renderCommunity==='function')renderCommunity()}catch(e){}try{if(typeof renderMyPage==='function')renderMyPage()}catch(e){}decorate()}
window.YeorinAvatar={
  refresh:refresh,applyMap:applyMap,url:function(id){const p=pathOf(id);return p?publicUrl(p):''},
  open:function(){style();const mine=me(),cur=window.YA({id:mine.id||''});const m=document.createElement('div');m.className='ya-modal';m.id='yaSheet';m.onclick=e=>{if(e.target===m)m.remove()};m.innerHTML='<div class="ya-sheet"><button class="ya-close" onclick="yaSheet.remove()">×</button><h3>프로필 사진</h3><div class="ya-preview" id="yaPreview">'+cur+'</div><div style="font-size:12px;color:var(--ink3)">5MB 이하 JPG · PNG · WEBP</div><input id="yaFile" type="file" accept="image/jpeg,image/png,image/webp" style="margin:12px 0;width:100%"><button class="ya-primary" onclick="YeorinAvatar.save()">사진 저장</button>'+(pathOf(mine.id)?'<button class="ya-ghost" onclick="YeorinAvatar.clear()">사진 지우고 기본 프로필로 돌아가기</button>':'')+'</div>';document.body.appendChild(m);const f=document.getElementById('yaFile');f.onchange=()=>{const file=f.files&&f.files[0];if(!file)return;const u=URL.createObjectURL(file),preview=document.getElementById('yaPreview');if(preview)setHtml(preview,'<img class="ya-img" src="'+u+'">')}},
