...
async function copyDir(src,dst){
  ...
  for(const name of await readdir(src)){
    ...
    else if(!['index.html','build.mjs','package.json','vercel.json','ui-patch.mjs','date-speed-patch.mjs','supabase-native.js','gas-zero-patch.js','community.js','community-image-fix.js','community-editor-fix.js','app-ux-fix.js','pwa-update.js','exit-hint-timeout.js','nanum-time-fix.js','notification-push.js','push-ios-story-fix.js','nanum-v2.js','bible-provider.js','bible-reader.js','bible-back-fix.js','bible-progress-sync-fix.js','bible-reader-link-fix.js','nanum-v2-compose-fix.js','nanum-range-patch.js','nanum-range-open-ended-fix.js','addressbook-fix.js','avatar.js','social-feed.js','story-picker-ui.js','nanum-ui-refresh.js','filter-control-polish.js','challenge-reminder-ui.js','challenge-push-nav.js'].includes(name))await copyFile(from,to);
  }
}
...
const inline=[native,gasZero,community].map(code=>code.replace(/<\/script/gi,'<\\/script')).join('\n\n')
  +'\n\n'+[communityEditor,bibleBack,appUx,pwaUpdate,addressbookFix,exitHint,bibleProvider,bibleReader,bibleProgressSync,bibleReaderLinkFix,nanumV2,nanumV2ComposeFix,nanumRangePatch,nanumRangeOpenEndedFix,nanumTime,avatar,socialFeed,storyPicker,nanumUiRefresh,filterControlPolish,challengeReminder,push,pushIosStoryFix,challengePushNav].map(code=>code.replace(/<\/script/gi,'<\\/script')).join('\n\n');
const tag='<script>\n'+inline+'\n</script>';
