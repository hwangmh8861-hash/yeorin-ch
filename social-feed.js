...
async function load(reset){
  ...
  try{
    const tasks=[rpc('yeorin_social_feed',{p_kind:state.kind,p_author_id:state.author||null,p_page:state.page,p_per_page:15})];
    if(reset)tasks.push(rpc('yeorin_story_feed',{}));
    const res=await Promise.all(tasks),f=res[0]||{},incoming=f.posts||[];
    if(reset)state.stories=res[1]||[];
...
function storyStrip(){
  const order=storyOrder(),me=uid(),mine=order.find(x=>x.authorId===me),myEmoji=esc((typeof CU!=='undefined'&&CU&&CU.emoji)||'🌿');
  let h='<div class="ys-stories">';
  if(mine){
    const allSeen=mine.list.every(s=>s.viewed);
    h+='<button class="ys-story'+(allSeen?' seen':'')+'" onclick="openStoryUser(\''+esc(me)+'\')"><div class="ys-ring"><div class="ys-face">'+myEmoji+'</div><span class="ys-plus" onclick="event.stopPropagation();openStoryUploader()">+</span></div><div class="ys-name">내 스토리</div></button>'
  }else{
    h+='<button class="ys-story" onclick="openStoryUploader()"><div class="ys-ring empty"><div class="ys-face">'+myEmoji+'</div><span class="ys-plus">+</span></div><div class="ys-name">내 스토리</div></button>'
  }
  order.filter(x=>x.authorId!==me).forEach(x=>{
    const a=(x.list[0]&&x.list[0].author)||{},allSeen=x.list.every(s=>s.viewed);
    h+='<button class="ys-story'+(allSeen?' seen':'')+'" onclick="openStoryUser(\''+esc(x.authorId)+'\')"><div class="ys-ring"><div class="ys-face">'+esc(a.emoji||'🌿')+'</div></div><div class="ys-name">'+esc(a.name||'')+'</div></button>'
  });
  return h+'</div>'
}
...
/* ---------- 렌더 ---------- */
function people(){const users=(typeof AD!=='undefined'&&AD&&AD.users)||[];let h='<div class="ys-people"><button class="ys-person '+(!state.author?'on':'')+'" onclick="socialPerson(\'\')">전체 사람</button>';users.forEach(u=>h+='<button class="ys-person '+(state.author===u.id?'on':'')+'" onclick="socialPerson(\''+esc(u.id)+'\')">'+esc(u.emoji||'')+' '+esc(u.name)+'</button>');return h+'</div>'}
...
function render(){
  ...
  const filters=['all','qt','prayer'].map(k=>'<button class="ys-chip '+(state.kind===k?'on':'')+'" onclick="socialKind(\''+k+'\')">'+({all:'전체',qt:'나눔',prayer:'기도'}[k])+'</button>').join('');
  ...
  r.innerHTML='<div class="ys-head">'+storyStrip()+'<div class="ys-filter">'+filters+'</div>'+people()+'</div><div class="ys-feed">'+posts+'<div id="ysSentinel" class="ys-sentinel idle"><i></i></div></div><button class="ys-write" onclick="openSocialWrite()">＋</button>';
  syncObserver()
}

/* ---------- 필터 / 액션 ---------- */
window.socialKind=k=>{if(state.kind===k)return;state.kind=k;load(true)};
window.socialPerson=id=>{if(state.author===id)return;state.author=id;load(true)};
...
window.socialComment=async function(kind,id){
  ...
  try{
    ...
    if(p){
      ...
      p.comments.push({id:uuid(),authorId:uid(),author:{id:uid(),name:(typeof CU!=='undefined'&&CU&&CU.name)||'',emoji:(typeof CU!=='undefined'&&CU&&CU.emoji)||''},text:text,date:new Date().toISOString().slice(0,10),createdAt:new Date().toISOString()});
      replacePost(p)
...
window.submitStory=async function(){
  ...
  try{
    ...
    console.error(e);
    if(uploaded){try{await rpc('yeorin_story_delete',{p_story_id:id})}catch(_e){}await removeMedia(path);showToast('스토리 저장에 실패해서 올린 파일을 되돌렸어요','error')}
